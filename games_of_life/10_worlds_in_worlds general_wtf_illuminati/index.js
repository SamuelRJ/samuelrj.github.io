...
const addThing1 = (fieldAt) => {
  if (fieldAt.vel < 0.1 && fieldAt.vel > -0.1 && fieldAt.vel != 0) {
    let xPos = fieldAt.x;
    let yPos = fieldAt.y;
    // fieldAt.pos += Math.abs(Math.log(Math.abs(fieldAt.vel) + 0.01));
    fieldAt.vel +=
      ((height - mouseY) / 100) *
      Math.tan(Math.sin(xPos / 10) + yPos + clock / 1.01);
  }
};

const addThing2 = (fieldAt) => {
  if (Math.abs(fieldAt.pos) > 100 && Math.abs(fieldAt.vel) >= 0) {
    fieldAt.vel *= -0.9;
  }
};
