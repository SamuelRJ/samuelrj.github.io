...
const mouseControlElectron = () => {
  for (let i = 0; i < 1; i++) {
    let vxTarget = Math.round(mouseX - electron[i].x[clock - 1]) / 30;
    let vyTarget = Math.round(mouseY - electron[i].y[clock - 1]) / 30;
    let vTarget = (vxTarget ** 2 + vyTarget ** 2) ** 0.5;
    if (vTarget > speedOfLight) {
      let vTargetScale = vTarget / (speedOfLight * 0.9);
      vxTarget /= vTargetScale;
      vyTarget /= vTargetScale;
    }
    electron[i].velX -= Math.round(100 * (electron[i].velX - vxTarget)) / 500;
    electron[i].velY -= Math.round(100 * (electron[i].velY - vyTarget)) / 500;
    electron[i].x[clock] = electron[i].x[clock - 1] + electron[i].velX;
    electron[i].y[clock] = electron[i].y[clock - 1] + electron[i].velY;
  }
};
...
const updatePixelValue = (x, y) => {
  ...
  for (let i = 0; i < electron.length; i++) {
    ...
    if (d2e < 400) {
      ...
      let force = calculateDistances(x, y, electron[i], lastDistance);
  ...
  while (frameCounter++ < 1000) {
    ...
    if (distAt - frameCounter * speedOfLight < 0) {
      ...
      let calcdDistance =
        ...
        0.3 *
          (distPrev - distAt) *
          ((timeAtFrame - distPrev) / (distAt - distPrev + 0.00001));
      ...
      pixelData[fromX][fromY].lastDistToE[toElectron.id] = calcdDistance;
...
const nextTick = () => {
  if (clock < 1000) {
    electron[0].velX = Math.sin(clock / 12) / 2;
    electron[0].velY =
      Math.cos(
